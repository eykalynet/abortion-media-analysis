...
get_visnetwork_data <- function(df, outlet_input, period_input) {
  ...
  topics <- df |> 
    distinct(id = as.character(topic), label = topic_name) |> 
    mutate(group = "Topic", color = topic_color, value = 40)

  bigrams <- df |> 
    distinct(id = as.character(term), label = term) |> 
    mutate(group = "Bigram", color = bigram_color, value = 20)

  nodes <- bind_rows(topics, bigrams)

  edges <- df |> transmute(
    from = as.character(topic),   # fixed
    to = as.character(term),      # fixed
    value = beta,
    width = beta * 10,
...
save_visnetwork_json <- function(df, outlet_input, period_input) {
  ...
  write_json(net, path = file.path("../data/wrangled/networks", filename), pretty = TRUE, auto_unbox = TRUE)
}
