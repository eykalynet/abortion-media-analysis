...
# Function to generate nodes and edges for a given outlet + period
export_network_json <- function(df, outlet_input, period_input) {
  df <- df |> filter(outlet == outlet_input, period == period_input)

  # Topic colors (darker, since they're parent nodes)
  topic_color <- case_when(
    outlet_input == "Fox" & period_input == "Pre-Roe"  ~ "#c35b5b",  # medium red
    outlet_input == "Fox" & period_input == "Post-Roe" ~ "#6a040f",  # deep red
    outlet_input == "NYT" & period_input == "Pre-Roe"  ~ "#61a5c2",  # medium blue
    outlet_input == "NYT" & period_input == "Post-Roe" ~ "#003049",  # deep navy
    TRUE ~ "#cccccc"
  )

  # Bigram colors (lighter than topics for visual contrast)
  bigram_color <- case_when(
    outlet_input == "Fox" & period_input == "Pre-Roe"  ~ "#f4a5a5",  # light red
    outlet_input == "Fox" & period_input == "Post-Roe" ~ "#9e2a2b",  # dark red
    outlet_input == "NYT" & period_input == "Pre-Roe"  ~ "#a9d6e5",  # light blue
    outlet_input == "NYT" & period_input == "Post-Roe" ~ "#1d3557",  # dark blue
    TRUE ~ "#999999"
  )
  ...
  topics <- df |> 
    ...
    mutate(
      ...
      category = "Topic",
      symbolSize = 40,
      itemStyle = map(topic_color, ~ list(color = .x))
    )
  ...
  bigrams <- df |> 
    ...
    mutate(
      ...
      category = "Bigram",
      symbolSize = 20,
      itemStyle = map(bigram_color, ~ list(color = .x))
    )

  # Combine into one node list
  nodes <- bind_rows(topics, bigrams) |> 
    mutate(id = name) |> 
    select(id, name, category, symbolSize, itemStyle)
  ...
  links <- df |> transmute(
    ...
    target = term,
    value = beta,
    lineStyle = list(width = beta * 100)
  )

  list(nodes = nodes, links = links)
}

# Function to save JSON file to disk
save_json <- function(df, outlet_input, period_input) {
  net <- export_network_json(df, outlet_input, period_input)
  filename <- paste0("network_", tolower(outlet_input), "_", tolower(gsub("-", "", period_input)), ".json")
  write_json(net, file.path("../data/wrangled/networks", filename), pretty = TRUE, auto_unbox = TRUE)
}

# Export all four networks
save_json(all_topics_named, "Fox", "Pre-Roe")
save_json(all_topics_named, "Fox", "Post-Roe")
save_json(all_topics_named, "NYT", "Pre-Roe")
save_json(all_topics_named, "NYT", "Post-Roe")
```
