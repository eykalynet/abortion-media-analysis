...
```{r}
tokens_bigrams2 <- bind_rows(
  nyt |> mutate(outlet = "NYT"),
  fox |> mutate(outlet = "Fox")
) |>
  mutate(week = lubridate::floor_date(as.Date(pub_date), "week")) |>
  ...
  count(week, bigram, sort = TRUE)

top_bigrams_weekly <- tokens_bigrams2 |>
  group_by(week) |>
  slice_max(n, n = 15, with_ties = FALSE) |>
  mutate(rank = row_number()) |>
  ungroup()

p <- ggplot(top_bigrams_weekly, aes(x = -rank, y = n, fill = bigram)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = bigram, y = 0), hjust = 1, size = 4) +
  coord_flip(clip = "off") +
  scale_x_continuous(breaks = NULL) +
  labs(
    title = "Top Bigrams in News Articles ({closest_state})",
    subtitle = "Weekly Frequency of Bigrams | NYT & Fox",
    x = NULL, y = "Count"
  ) +
...
animate(p, fps = 10, width = 800, height = 600, duration = 30, renderer = gifski_renderer("bigrams_bar_chart_race.gif"))
