...
# Prepare interactive tooltip columns (do once)
monthly_data_2 <- monthly_data |>
  mutate(
    ...
    data_id_c = paste(outlet, "Cumulative", format(month, "%Y-%m"), cumulative_n, sep = "-")
  )

# Split data
fox_data_m <- monthly_data_2 |> filter(outlet == "Fox News")
nyt_data_m <- monthly_data_2 |> filter(outlet == "The New York Times")

fox_data_c <- fox_data_m
nyt_data_c <- nyt_data_m
```
...
monthly_plot <- ggplot() +
  geom_area_interactive(
    data = fox_data_m,
    aes(x = month, y = monthly_n, fill = outlet, tooltip = tooltip_text_monthly, data_id = data_id_monthly),
    alpha = 0.35
  ) +
  ...
  geom_vline(xintercept = as.Date("2022-06-24"), linetype = "dashed", color = "grey80", linewidth = 0.8) +
  scale_fill_manual(values = fill_colors) +
  scale_color_manual(values = line_colors) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b'%y") +
  labs(title = "Monthly Volume of Abortion Coverage", x = "Date", y = "Number of Articles", fill = "Outlet", color = "Outlet") +
...
cumulative_plot <- ggplot() +
  geom_area_interactive(
    data = fox_data_c,
    aes(x = month, y = cumulative_n, fill = outlet, tooltip = tooltip_text_c, data_id = data_id_c),
    alpha = 0.35
  ) +
  geom_line_interactive(
    data = fox_data_c,
    aes(x = month, y = cumulative_n, color = outlet, tooltip = tooltip_text_c, data_id = data_id_c),
    linewidth = 1.2
  ) +
  geom_point_interactive(
    data = fox_data_c,
  ...
  scale_color_manual(values = line_colors) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b'%y") +
  labs(title = "Cumulative Abortion Coverage Over Time", x = "Date", y = "Cumulative Number of Articles", fill = "Outlet", color = "Outlet") +
  theme_light(base_family = "News Cycle") +
  theme(
    legend.position = "top",
    legend.title = element_blank(),
...
To understand how language and sentiment around abortion shifted in the media before and after the Supreme Court’s landmark decision in *[Dobbs v. Jackson Women’s Health Organization](https://www.oyez.org/cases/2021/19-1392)*, we mapped article [AFINN](https://en.wikipedia.org/wiki/AFINN) sentiment scores across the two major outlets in a calendar heatmap format spanning 2020 to 2024.
