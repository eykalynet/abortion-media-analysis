...
calendar <- merge(calendar, fox_sentiment, by = "date", all.x = TRUE)
calendar[, sentiment_score := na.approx(sentiment_score, na.rm = FALSE)]
calendar[is.na(sentiment_score), sentiment_score := 0]
...
calendar[is.na(week) | week <= 0, week := 1]

# Create labels for month axis
MonthLabels <- calendar[, .(meanWkofYr = mean(week)), by = month][
  , month := month.abb[month]
]

# Dynamically compute sentiment range for color scale
range_max <- ceiling(max(abs(calendar$sentiment_score), na.rm = TRUE))

# Plot final calendar heatmap
ggplot(data = calendar, aes(date = date, fill = sentiment_score)) + 
