...
# Add weekend and month-related effects
...
dat[, ':='(
  ...
  week = as.integer(strftime(date, "%V")) # ISO week, Monday start
)]

# Fill missing ValueCol if needed
dat[, ValueCol := na.approx(ValueCol, na.rm = FALSE)]

# Create month labels
MonthLabels <- dat[, .(meanWkofYr = mean(week)), by = month][
...
ggplot(data = calendar_fox, aes(date = date, fill = sentiment_score)) + 
  stat_calendar_heatmap(colour = "grey80", size = 0.1) + 
  scale_fill_scico(
    palette = "vik",
...
# Step 1: Tokenize and assign article ID
...
# Step 2: Join with AFINN sentiment lexicon
afinn <- get_sentiments("afinn")
...
# Step 3: Floor timestamps, then average per day
...
# Step 4: Create calendar covering all real article dates
calendar_nyt <- data.table(
  date = seq(min(nyt_sentiment$date), max(nyt_sentiment$date), by = "day")
)

# Step 5: Merge + interpolate missing values
...
# Step 7: Create labels for month axis
MonthLabels_nyt <- calendar_nyt[, .(meanWkofYr = mean(week)), by = month][
  , month := month.abb[month]
]

# Step 8: Dynamically compute sentiment range for color scale
range_max_nyt <- ceiling(max(abs(calendar_nyt$sentiment_score), na.rm = TRUE))

# Step 9: Plot final calendar heatmap
ggplot(data = calendar_nyt, aes(date = date, fill = sentiment_score)) + 
  ...
  scale_x_continuous(
    name = NULL,
    breaks = MonthLabels_nyt$meanWkofYr,
    labels = MonthLabels_nyt$month,
    expand = c(0, 0)
  ) + 
  facet_wrap(~Year, ncol = 1, strip.position = "right") + 
  theme(
