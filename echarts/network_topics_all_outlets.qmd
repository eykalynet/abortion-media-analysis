...
build_network_plot <- function(outlet, period, title_text) {
  ...
  # Build visNetwork plot with a title
  htmltools::tagList(
    htmltools::tags$h3(title_text, 
      style = "font-family: 'News Cycle'; text-align: center; margin-bottom: 25px;"),
...
save_network_plot <- function(plot_obj, outlet, period) {
  output_file <- sprintf("network_%s_%s.html", 
                         tolower(outlet), tolower(gsub("-", "", period)))

  # Save only the visNetwork part of the tagList (2nd element)
  htmlwidgets::saveWidget(plot_obj[[2]], file = output_file, background = "white")
}
...
for (outlet in outlets) {
  for (period in periods) {
    ...
    save_network_plot(plot_obj, outlet, period)
  }
}
