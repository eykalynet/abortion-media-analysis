...
render_and_save_network <- function(outlet, period, title_text) {
  ...
  network_data$nodes$shapeProperties <- lapply(seq_len(nrow(network_data$nodes)), function(i) list(borderRadius = 10))
  network_data$nodes$font <- lapply(seq_len(nrow(network_data$nodes)), function(i) list(size = 16, face = "News Cycle", multi = "html"))
