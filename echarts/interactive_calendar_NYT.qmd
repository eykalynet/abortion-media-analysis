...
calendar_nyt <- merge(calendar_nyt, nyt_sentiment, 
                      by = "date", all.x = TRUE)

# Interpolate missing sentiment scores linearly (for visualization 
# continuity)
calendar_nyt[, sentiment_score := na.approx(sentiment_score, 
                                            na.rm = FALSE)]
...
calendar_nyt <- merge(calendar_nyt, event_dates, 
                      by = "date", all.x = TRUE)
...
calendar_plot_nyt <- ggplot(calendar_nyt, aes(x = weekofyear, y = 8 - weekday)) +
  ...
  labs(title = "Sentiment Heatmap of The New York Times Abortion Articles (2020–2024)") +
...
save_nyt_chart <- function(chart, file_path = "nyt_sentiment_calendar.html") {
