...
fox_meta5 <- fox_meta3 |> 
  ...
  select(-url) |> 
  mutate(np_po = if_else(sentiment_score > 3, "positive", "zero"))|>
  mutate(np_po = if_else(sentiment_score < -3, "negative", np_po)) |> 
  ...
  mutate(tagging = if_else(str_detect(text, regex("catholic.*christian.*religion|catholic.*religion.*christian|christian.*catholic.*religion|christian.*religion.*catholic|religion.*catholic.*christian|religion.*christian.*catholic", ignore_case = TRUE)), if_else(is.na(tagging), "R", paste(tagging, "R", sep = ", ")), tagging))


g1 <- ggplot(fox_meta5, aes(x = `title`, y = `sentiment_score`, color = `roe_status`)) +
