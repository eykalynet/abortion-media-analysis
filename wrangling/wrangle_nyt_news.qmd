...
nyt_meta <- nyt_news_df |>
  ...
  as_tibble() |>
  transmute(
    `__typename`,                # Article type (e.g., "Article", "Video", etc.)
    title = promotionalHeadline,  # Article headline
    publicationDate = map_chr(firstPublished, ~ .x[1]),  # Use only the first timestamp
    url,                          # Full article URL
    description = creativeWorkSummary,  # Short article summary
    text                           # Full article body text
  ) |>
  # Keep only real articles
  filter(`__typename` == "Article") |>
  # Clean publication date format
  mutate(
